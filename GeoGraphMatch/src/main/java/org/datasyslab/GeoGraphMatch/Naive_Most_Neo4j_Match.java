...
public class Naive_Most_Neo4j_Match {
		...
		public Naive_Most_Neo4j_Match(String db_path)
		{
...
//		}
	     ...
	     * @return
	     */
	    public void SubgraphMatch_Spa_API(Query_Graph query_Graph, int limit)//use neo4j query
		{
			...
			try
			{
				...
				result_count = 0;
				ArrayList<String> spa_pred_str = new ArrayList<String>();
				...
				for(int i = 0; i < query_Graph.Has_Spa_Predicate.length; i++)
				{
					...
					{
						spa_pred_str.add(String.format("a%d", i));
						rectangles.add(query_Graph.spa_predicate[i]);
					}
				}
				while(result.hasNext())
				{
					...
					for (int i = 0; i < spa_pred_str.size(); i++)
					{
						Node node = (Node) row.get(spa_pred_str.get(i));
						double lon = Double.parseDouble(node.getProperty(lon_name).toString());
						double lat = Double.parseDouble(node.getProperty(lat_name).toString());
						MyRectangle rectangle = rectangles.get(i);
						if(lon > rectangle.max_x || lon < rectangle.min_x || lat > rectangle.max_y || lat < rectangle.min_y)
						{
							satisfy = false;
							break;
						}
	    ...
	    public Result Explain_SubgraphMatch_Spa_API(Query_Graph query_Graph, int limit)
		{
